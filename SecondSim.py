...
def process_data(row, base_df):
    ...
    backup_number = base_df[base_df['Primary Number']
                            == primary_number]['Backup Number']
    ...
    if base_file and target_file:
        if st.button('Запустить'):
            with st.spinner('Обработка файлов...'):
                ...
                results_found, results_exceptions, results_not_found = parallel_process(
                    base_file, target_file)
                processing_time = time.time() - start_time

                st.success(f"Обработка завершена за {
                           processing_time:.2f} секунд")

                # Создание итогового DataFrame
                result_df = pd.DataFrame(columns=[
                                         "Primary Number", "Backup Number", "Exception Primary", "Exception Backup", "Not Found"])

                # Заполнение результатов
                for primary, backup in results_found:
                    result_df = result_df.append({"Primary Number": primary, "Backup Number": backup,
                                                 "Exception Primary": None, "Exception Backup": None, "Not Found": None}, ignore_index=True)

                for primary in results_not_found:
                    result_df = result_df.append({"Primary Number": primary, "Backup Number": None,
                                                 "Exception Primary": None, "Exception Backup": None, "Not Found": primary}, ignore_index=True)

                # Сохранение результата
                result_df.to_excel('output.xlsx', index=False)
                st.download_button('Скачать результат', data=open('output.xlsx', 'rb').read(
                ...
                # Копирайт
                st.markdown(
                    "<p style='text-align: center;'>© zHeyBunny</p>", unsafe_allow_html=True)
